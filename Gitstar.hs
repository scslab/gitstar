...
server req lbody = do
  let routes = mconcat 
        ...
        , routeRestController "keys" $ KeysController lbody
        ...
        , routeMethod "GET" $ routeActionPattern "/:user_name/keys" $ restIndex $ KeysController lbody
  ...
  body <- unlabel lbody
  enumPure body |$ runLHttpRoute routes req
